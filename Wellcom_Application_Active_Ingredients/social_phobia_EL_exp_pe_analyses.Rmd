...
sum_stats_by_session_lsas <- mfqlsas_sessional_osca_long %>% 
  filter(measure == "lsas", time_point %in% seq(1:14)) %>% 
  group_by(time_point) %>% 
...
sum_stats_by_session_mfq <- mfqlsas_sessional_osca_long %>% 
  filter(measure == "mfq", time_point %in% seq(1:14)) %>% 
  group_by(time_point) %>% 
...
mean_plot_lsas <- ggplot(sum_stats_by_session_lsas, aes(x=reorder(time_point, seq(1:14)), y=avg_lsas, group = "condition")) + 
...
mean_plot_mfq <- ggplot(sum_stats_by_session_mfq, aes(x=reorder(time_point, seq(1:14)), y=avg_mfq, group = "condition")) + 
