...
**Create a long dataset too in order to do some plotting**
```{r}
osca_dep_anx_long <- osca_dep_anx  %>%      
dplyr:: select(Prediction1Rating_Study2, ConditionOrder_2, Anxiety_with_Study2, Belief_With_Study2, Anxiety_WithOUT_Study2, Belief_WithOUT_Study2, pe_with ,pe_without)
  
osca_dep_anx_long   <- osca_dep_anx  %>% 
  pivot_longer(cols = c("LSAS_B_Total", "LSAS_M_Total", "LSAS_post_total", "SMFQ_B", "SMFQ_M", "SMFQ_post"), 
               names_to = c("lsas", "mfq"), values_to = c("values"), names_pattern = "new_?(.*)(.*)") %>% 
  as.data.frame()
...
```{r}
library(patchwork)
anx_change_plot / dep_change_plot #+ plot_layout(nrow = 2, byrow = FALSE)
```
