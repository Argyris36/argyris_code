...
dep_change_plot + geom_hline(yintercept = 10, linetype = "dashed") +
...
anx_change_plot <- anx_change_plot +
  facet_wrap(~ condition, ncol =2 )
anx_change_plot + geom_hline(yintercept = 35, linetype = "dashed") +
...
```{r}
soc_phobia_review <- read.csv("~/Downloads/Social_Phobi_Depression_outcomes - Sheet1.csv")
...
soc_phobia_review <- soc_phobia_review %>% 
  filter(time_point == "post") %>% 
  mutate(es_between_dep_CT_WL = 
           (dep_mean_CT - dep_mean_WL)
         /sqrt((dep_sd_CT^2 + dep_sd_WL^2))/2)



soc_phobia_review <- soc_phobia_review %>% 
  filter(time_point == "post") %>% 
  mutate(es_between_sp_CT_WL = 
...
joined_pre_post_df <- joined_pre_post_df %>% 
  mutate(es_within_mean_sp_CT = (sp_mean_CT - sp_mean_CT.1)/((sp_sd_CT + sp_sd_CT.1)/2))
