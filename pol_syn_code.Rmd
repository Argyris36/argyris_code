...
```{r echo = FALSE, message = FALSE, message=FALSE}
knitr:: kable(round(proportions(table(qs_pol_syn$diagnosis))*100, 2), col.names = c("diagnosis", "%"))
```
...
```{r}
lme_intercept <- lm(phq_total_score ~ time_point*patient_group + (1|pt_id),  data = qs_pol_syn_long)
summary(lme_intercept)


lme_intercept_slope <- lm(phq_total_score ~ time_point*patient_group + (1+ time_point|pt_id),  data = qs_pol_syn_long_for_imp)
summary(lme_intercept)
```
...
```{r}
#Let's do multiple imputation of the long dataset

# first some data wrangling to prepare the variables
# according to https://arxiv.org/pdf/2404.06967 
# cluster variable needs to be an integer
# here I am replacing the original 
pt_id_mapping <- qs_pol_syn_long %>%
  select(pt_id) %>%
  distinct() %>%
...
### Prepare the model for MI

```{r}
# create smaller dataset
qs_pol_syn_long_for_imp <- qs_pol_syn_long %>%
  dplyr:: select(
"pt_id_int.x", "art_site", "art_type", "gender", "patient_group", "diagnosis", 
"place_residence", "live_with_others", "phq_total_score", "gad_total_score", 
"wembs_total_score", "time_point", "age_intake"
  ) %>% 
  mutate(pt_id = pt_id_int.x)  %>% 
  select(-pt_id_int.x) #%>%
#  rename(pt_id = pt_id_int)
```
...
predictors <- c("art_site", "art_type", "gender", "patient_group", "diagnosis", 
...
# Ensure 'pt_id' and 'patient_id' are not imputed
pred_matrix["pt_id", ] <- 0


# Specify methods for imputation using miceadds
methods <- make.method(qs_pol_syn_long_for_imp)

# Specify multilevel imputation with random slopes for the outcome variables
methods["phq_total_score"] <- "2l.pan"
methods["gad_total_score"] <- "2l.pan"
methods["wembs_total_score"] <- "2l.pan"

# Custom function for imputation with miceadds
miceadds_impute_pan <- function(y, ry, x, type, ...) {
  # Specify 'pt_id' as the level-2 predictor
  type <- rep(1, ncol(x) + 1)  # Default to level-1 predictor
  type[which(colnames(x) == "pt_id")] <- 2  # Set 'pt_id' as the level-2 predictor
  
  # Specify 'time_point' as the random slope
  if ("time_point" %in% colnames(x)) {
    type[which(colnames(x) == "time_point")] <- 3  # Set 'time_point' as random slope
  }

  miceadds::mice_impute_pan(y, ry, x, type = type, random_slope = TRUE, ...)
}
...
imputed_data <- mice(
  ...
  seed = 123,
  visitSequence = "monotone", # Optional: specify visit sequence
  imputationFunction = list(
    phq_total_score = miceadds_impute_pan,
    gad_total_score = miceadds_impute_pan,
    wembs_total_score = miceadds_impute_pan
  )
)
...
# Obtain the completed data set after imputation
...
# Specify multilevel imputation with random intercepts for the outcome variables
methods["phq_total_score"] <- "2l.pan"
methods["gad_total_score"] <- "2l.pan"
methods["wembs_total_score"] <- "2l.pan"
...
miceadds_impute_pan <- function(y, ry, x, type, ...) {
  ...
  miceadds::mice_impute_pan(y, ry, x, type = type, random_slope = FALSE, ...)
}
...
qs_pol_syn_long_for_imp <- qs_pol_syn_long %>%
  dplyr:: select(
"pt_id_int", "art_site", "art_type", "gender", "patient_group", "diagnosis", 
"place_residence", "live_with_others", "phq_total_score", "gad_total_score", 
"wembs_total_score", "time_point", "age_intake"
  ) %>% 
  mutate(pt_id = pt_id_int)  %>% 
  select(-pt_id_int) #%>%
#  rename(pt_id = pt_id_int)

# apparently pt_id needs to be a factor
#qs_pol_syn_long_for_imp$pt_id <- as.factor(qs_pol_syn_long_for_imp$pt_id)
...
# Set up parallel processing, leaving one core free
plan(multisession, workers = num_cores - 1)

# Print the number of workers being used
print(paste("Using", num_cores - 1, "workers for parallel processing"))

# Helper function for parallel processing with MICE
future_mice <- function(..., future.globals = TRUE) {
  future.apply::future_lapply(1:5, function(i) mice(..., seed = i), future.globals = future.globals)
}

# Example use of the future_mice function
imputed_data_list <- future_mice(
  qs_pol_syn_long_for_imp, 
  m = 5, 
  maxit = 10,  # Reduced from 50 to 10 for speed
  method = methods, 
  predictorMatrix = pred_matrix, 
...
imputed_data <- mice::complete(imputed_data_list, action = "long")
