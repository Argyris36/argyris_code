...
```{r warning = F, message = F}
eipes_try <- read_excel("~/Downloads/ΕΡΕΥΝΑ ΓΙΑ ΤΗΝ ΙΑΤΡΙΚΗ ΠΑΙΔΕΙΑ ΚΑΙ ΕΡΓΑΣΙΑ (ΕΙΠΕς) (Απαντήσεις) (13).xlsx")
...
barplot(gender_tab,main = "Φύλο", col = "skyblue" )
bp_gender <- barplot(gender_tab,main = "Φύλο", col = "skyblue",ylim = c(0, max(gender_tab) + 0.05))
text(bp_gender, gender_tab, labels = paste0(counts_gender, " (", round(location * 100, 1), "%)"), pos = 3, cex = 0.8, col = "black")
...
# Mean Age
summary(eipes_try$Ηλικία)

# Age Histogram
hist(eipes_try$Ηλικία, main = "Ηλικία" , col = "skyblue", xlab = "")
...
bp <- barplot(location, main = "Πού Κάνεις Ειδικότητα" , col = "skyblue",ylim = c(0, max(location) + 0.05))
text(bp, location, labels = paste0(counts_location, " (", round(location * 100, 1), "%)"), pos = 3, cex = 0.8, col = "black")
...
barplot(t(counts_matrix), 
        ...
        main = "Daily Counts for AM and PM", 
        xlab = "", 
        ylab = "Counts", 
        legend.text = c("AM", "PM"),
